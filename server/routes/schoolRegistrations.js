...
router.post('/', async (req, res) => {
  try {
    const { cycle, plan, amount, schoolName, directorName, country, city, neighborhood, paymentMethods, whatsapp, email } = req.body
    ...
    if (!emailRegex.test(email)) {
      return res.status(400).json({ message: 'Veuillez fournir un email Gmail valide' })
    }
