...
export default function TeacherSalaryPage() {
  ...
  const summary = salariesQ.data?.summary || { total: 0, totalPaid: 0, totalPending: 0 }
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      <div>
        ...
        <p className="text-sm text-gray-500">Historique de vos salaires : combien et quand vous avez été payé</p>
      </div>

      <div className="grid grid-cols-3 gap-3">
        <div className="card p-4"><div className="w-10 h-10 rounded-xl flex items-center justify-center mb-3 bg-blue-100"><Banknote size={18} className="text-blue-600" /></div><div className="text-lg sm:text-2xl font-bold text-gray-900">{fmt(summary.total)}</div><div className="text-xs text-gray-500 mt-0.5">Total</div></div>
        <div className="card p-4"><div className="w-10 h-10 rounded-xl flex items-center justify-center mb-3 bg-green-100"><CheckCircle2 size={18} className="text-green-600" /></div><div className="text-lg sm:text-2xl font-bold text-gray-900">{fmt(summary.totalPaid)}</div><div className="text-xs text-gray-500 mt-0.5">Reçu</div></div>
        ...
        {loading ? (
          ...
          <table className="w-full min-w-[560px]">
            <thead><tr className="border-b border-gray-100 bg-gray-50">
              {['Mois', 'Montant', 'Statut', 'Payé le', 'Méthode'].map((h) => <th key={h} className="text-left text-xs font-semibold text-gray-500 px-4 py-3">{h}</th>)}
            </tr></thead>
            <tbody className="divide-y divide-gray-50">
              {salaries.map((s) => (
                <tr key={s._id} className="hover:bg-gray-50">
                  <td className="px-4 py-3 text-sm text-gray-900 capitalize">{monthLabel(s.month)}</td>
                  <td className="px-4 py-3 text-sm font-semibold text-gray-900 whitespace-nowrap">{fmt(s.amount)}</td>
