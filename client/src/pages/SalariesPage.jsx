...
const emptyForm = () => ({ teacherId: '', month: currentMonth(), amount: '', status: 'pending', method: 'cash', reference: '', note: '' })
...
export default function SalariesPage() {
  ...
  const openEdit = (s) => {
    setEditing(s)
    setForm({ teacherId: s.teacher?._id || s.teacher, month: s.month, amount: s.amount, status: s.status, method: s.method || 'cash', reference: s.reference || '', note: s.note || '' })
    setError(''); setModalOpen(true)
  }

  const save = async () => {
    if (!form.teacherId || !form.month || form.amount === '' || Number(form.amount) < 0) { setError('Enseignant, mois et montant valides requis'); return }
    setSaving(true); setError('')
    try {
      if (editing) await salariesApi.update(editing._id, form)
      else await salariesApi.create(form)
        ...
        {loading ? (
          ...
          <table className="w-full min-w-[720px]">
            <thead><tr className="border-b border-gray-100 bg-gray-50">
              {['Enseignant', 'Mois', 'Montant', 'Statut', 'Payé le', 'Méthode', ''].map((h) => <th key={h} className="text-left text-xs font-semibold text-gray-500 px-4 py-3">{h}</th>)}
            </tr></thead>
            <tbody className="divide-y divide-gray-50">
              {salaries.map((s) => (
                <tr key={s._id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-xs text-gray-500">{METHODS.find((m) => m.value === s.method)?.label || s.method}</td>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-card-lg w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-5 space-y-3">
              ...
              <p className="text-[11px] text-gray-400">Un salaire marqué « Payé » est automatiquement ajouté aux dépenses (Factures).</p>
            </div>
