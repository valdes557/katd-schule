...
import {
  Newspaper, Briefcase, MapPin, Clock, X, Loader2, ArrowLeft, Send,
  CheckCircle2, Upload, Building2,
} from 'lucide-react'
...
export default function NewsPage() {
  ...
  useEffect(() => {
    (async () => {
      try { const r = await recruitmentApi.publicList(); setPosts(r.data || []) } catch (_) {}
      setLoading(false)
    })()
  ...
  const openPost = (p) => { setSelected(p); setForm(EMPTY_APP); setSent(false) }
  ...
  const list = (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        ...
        <div>
          <h1 className="text-xl font-bold text-gray-900">Actualités & Recrutement</h1>
          <p className="text-sm text-gray-500">Les dernières offres publiées par les établissements</p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-14 text-gray-400">
          ...
          <p className="text-sm">Aucune annonce de recrutement pour le moment.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {posts.map((p) => (
            <button key={p._id} onClick={() => openPost(p)} className="card p-4 text-left hover:shadow-md transition-shadow">
              <div className="flex items-center gap-2 mb-2">
                {p.school?.logo
                  ? <img src={p.school.logo} alt="" className="w-8 h-8 rounded-lg object-cover" />
                  : <span className="w-8 h-8 rounded-lg bg-amber-100 flex items-center justify-center text-amber-600"><Building2 size={16} /></span>}
              ...
              <div className="flex items-center gap-3 mt-3 text-xs text-gray-400">
                ...
                {p.deadline && <span className="flex items-center gap-1"><Clock size={12} /> {new Date(p.deadline).toLocaleDateString('fr-FR')}</span>}
              </div>
  ...
  // Rendu : dans /u l'en-tête vient de UserLayout ; sinon page publique autonome.
  const body = (
    <>
      {list}
      ...
      {selected && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-card-lg w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto">
            ...
            <p className="text-xs text-gray-500 mb-1">{selected.school?.name} · {[selected.poste, selected.location, selected.contractType].filter(Boolean).join(' · ')}</p>
