...
export default function AdminSchoolSubjectsPage() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    if (!schoolId) { alert('Veuillez sélectionner une école'); return }
    setSubmitting(true)
    try {
  ...
  const toggleClass = (cid) => {
    setForm((f) => ({
      ...f,
      classes: f.classes.includes(cid) ? f.classes.filter((c) => c !== cid) : [...f.classes, cid],
    }))
  }
  ...
  if (loadingSchools) {
    return <div className="flex justify-center py-24"><Loader2 size={28} className="animate-spin text-blue-600" /></div>
  }
    ...
    <div className="space-y-5 animate-fade-in">
      ...
      <div>
        <h1 className="text-xl font-bold text-gray-900 flex items-center gap-2">
          <ClipboardList size={22} className="text-purple-600" /> Matières par école
        </h1>
        <p className="text-sm text-gray-500">Sélectionnez une école pour gérer ses matières et programmes.</p>
      </div>

      {/* School selector */}
      <div className="card p-4 space-y-3">
        <label className="text-xs font-semibold text-gray-700 flex items-center gap-1.5"><SchoolIcon size={13} /> École à gérer</label>
        {schools.length === 0 ? (
          <p className="text-sm text-gray-400">Aucune école disponible</p>
        ) : (
          <select value={schoolId} onChange={(e) => setSchoolId(e.target.value)} className="input text-sm w-full">
            {schools.map((s) => <option key={s._id} value={s._id}>{s.name}{s.city ? ` · ${s.city}` : ''}</option>)}
          </select>
        )}
        {currentSchool && (
          <p className="text-xs text-gray-500">
            <strong>{subjects.length}</strong> matière(s) enregistrée(s) · <strong>{classes.length}</strong> classe(s) · <strong>{teachers.length}</strong> enseignant(s)
          </p>
        )}
      </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
          <div className="flex flex-wrap gap-2 flex-1">
            <div className="relative flex-1 min-w-[200px]">
              ...
              <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Rechercher une matière..." className="input pl-9 text-sm" />
            </div>
          ...
          <button onClick={() => { setEditing(null); setForm(EMPTY); setShowModal(true) }} className="btn-primary text-sm whitespace-nowrap">
      ...
      ) : !schoolId ? null : filtered.length === 0 ? (
        <div className="text-center py-16 text-gray-400">
          <AlertCircle size={36} className="mx-auto mb-3 opacity-30" />
          <p>Aucune matière enregistrée pour cette école</p>
          <button onClick={() => { setEditing(null); setForm(EMPTY); setShowModal(true) }} className="btn-primary text-sm mt-4">
            <Plus size={14} /> Créer la première matière
          </button>
        </div>
      ) : (
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-card-lg w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto">
            ...
            <p className="text-xs text-gray-500 mb-4">École : <strong>{currentSchool?.name}</strong></p>
            <form onSubmit={handleSubmit} className="space-y-3">
              ...
              <div>
                ...
                {teachers.length === 0 && <p className="text-[10px] text-gray-400 mt-1">Aucun enseignant pour cette école</p>}
              </div>
              <div>
                <label className="text-xs font-medium text-gray-600 mb-1 block">Classes associées</label>
                {classes.length === 0 ? (
                  <p className="text-[11px] text-gray-400 bg-gray-50 p-2 rounded">Aucune classe pour cette école</p>
                ) : (
